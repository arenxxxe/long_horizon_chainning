...
from chaining_package.ALGO.viskill_agent.algo.viskill_agents.utils.general_utils import (AverageMeter, Every, Timer, Until,
                                   set_seed_everywhere)
...
class SkillLearningTrainer(BaseTrainer):
    ...
    def train(self):
        ...
        while train_until_episode(self.global_episode):
            ...
            #监控这几个内存情况
            memory_trace={
                "agent的内存":asizeof.asizeof(self.agent),
                "train_sampler的内存":asizeof.asizeof(self.train_sampler),
                "buffer的内存": asizeof.asizeof(self.buffer),
                "eval sampler的内存": asizeof.asizeof(self.eval_sampler),
                "eval_env的内存" :asizeof.asizeof(self.eval_env),
                "train_env的内存" :asizeof.asizeof(self.train_env),
                "buffer_sampler的内存" :asizeof.asizeof(self.buffer_sampler),
                "demo_buffer的内存" :asizeof.asizeof(self.demo_buffer),
                "wandb的内存" :asizeof.asizeof(self.wb),
                "logger的内存" :asizeof.asizeof(self.logger),

            }
            wandb.log(memory_trace)
