...
class SC_EVAL(EVAL):
    ...
    def _setup_logger(self):
        if self.is_chef:
            exp_name = f"sc测试_{self.cfg.task}_{self.cfg.agent.sc_agent.name}_{self.cfg.agent.sl_agent.name}_seed{self.cfg.seed}"
...
import hydra    
@hydra.main(version_base=None, config_path="../my_trainer/configs", config_name="skill_chaining")
def main(cfg):
